...
async function main() {
  ...
  const PausableContract = await hre.ethers.getContractFactory("PausableBridge");

  // Bridge contract
  const BridgeContract = await hre.ethers.getContractFactory("Bridge");

  // Erc20 Handler contract
  const DefaultMessageReceiverContract = await hre.ethers.getContractFactory("DefaultMessageReceiver");
  const ERC20HandlerContract = await hre.ethers.getContractFactory("ERC20Handler");

  // Fee handler contract
  const FeeRouterContract = await hre.ethers.getContractFactory("FeeHandlerRouter");
  const BasicFeeHandlerContract = await hre.ethers.getContractFactory("BasicFeeHandler");
  const PercentageFeeHandler = await hre.ethers.getContractFactory("PercentageERC20FeeHandler");
  ...
  const defaultMessageReceiverAddr = await defaultMessageReceiverInstance.getAddress()
  ...
  const feeRouterInstance = await FeeRouterContract.deploy(
    bridgeAddr
  );
  ...
  await defaultMessageReceiver.grantRole(
    await defaultMessageReceiver.SYGMA_HANDLER_ROLE(),
    erc20HandlerAddr
  );
